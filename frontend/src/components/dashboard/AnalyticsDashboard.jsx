  ...
  useEffect(() => {
    ...
    useEffect(() => {
      const fetchAuditLogs = async () => {
        try {
          const response = await fetch(`${API_BASE_URL}/scribe/audit-logs`);
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>
                <thead>
                  <tr style={{ background: COLORS.slate[100] }}>
                    ...
                    <th style={{ padding: '12px', textAlign: 'left', borderBottom: `1px solid ${COLORS.slate[200]}` }}>Note ID</th>
                    <th style={{ padding: '12px', textAlign: 'left', borderBottom: `1px solid ${COLORS.slate[200]}` }}>Claims</th>
                ...
                <tbody>
                  {auditLogs.map((log, idx) => (
                        <tr key={idx} style={{ borderBottom: `1px solid ${COLORS.slate[100]}` }}>
                          <td style={{ padding: '12px', fontFamily: 'monospace', fontSize: '11px' }}>{log.timestamp?.slice(11, 19) || '-'}</td>
                          <td style={{ padding: '12px' }}>Note Analysis</td>
                          <td style={{ padding: '12px', color: COLORS.slate[600] }}>{log.note_id}</td>
                          <td style={{ padding: '12px' }}>{log.claims_total} claims</td>
                          <td style={{ padding: '12px' }}>
                            <span style={{
                              padding: '2px 8px', borderRadius: '4px', fontSize: '10px', fontWeight: '600',
                              background: '#dcfce7', color: '#166534'
                            }}>
                              {log.review_level || 'Logged'}
                            </span>
                          </td>
                        </tr>
                  ))}
